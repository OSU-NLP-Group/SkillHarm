...
def _call_azure_openai(
    ...
    """Call Azure OpenAI API. `deployment` is the Azure deployment name (passed as `model`)."""
    try:
...
def analyze_skill(
    ...
    """Analyze skill content using an LLM (OpenAI, Azure OpenAI, or Bedrock Claude).

    Args:
        content: Extracted skill content.
        model: Model name or alias. Prefix with 'bedrock/' for Bedrock, 'azure/' for
               Azure OpenAI (the suffix is the Azure deployment name), or use a Claude
               alias like 'claude-opus-4-6'.
        api_key: API key. For OpenAI uses OPENAI_API_KEY; for Azure uses
                 AZURE_OPENAI_API_KEY; for Bedrock, AWS credentials from env.
        provider: Force provider: 'openai' | 'azure' | 'bedrock'. Auto-detected if omitted.
        aws_region: AWS region for Bedrock (default from AWS_REGION env var).
        azure_endpoint: Azure OpenAI endpoint (default from AZURE_OPENAI_ENDPOINT).
        azure_api_version: Azure API version (default from AZURE_OPENAI_API_VERSION
                           or OPENAI_API_VERSION; falls back to '2024-10-21').
    """
    ...
    if provider is None:
        if model.startswith("azure/"):
            provider = "azure"
        elif model.startswith("bedrock/") or model.startswith("claude") or resolved_model != model:
            provider = "bedrock"
        elif os.environ.get("AZURE_OPENAI_ENDPOINT") and not os.environ.get("OPENAI_API_KEY"):
            # Only Azure credentials present — default to Azure
            provider = "azure"
        else:
            provider = "openai"
    ...
    if provider == "bedrock":
        ...
        deployment = resolved_model.removeprefix("azure/")
        endpoint = azure_endpoint or os.environ.get("AZURE_OPENAI_ENDPOINT")
        if not endpoint:
            raise ValueError("AZURE_OPENAI_ENDPOINT not set (or pass azure_endpoint=)")
        api_key = api_key or os.environ.get("AZURE_OPENAI_API_KEY") or os.environ.get("AZURE_OPENAI_KEY")
        if not api_key:
        ...
        result_text, tokens = _call_azure_openai(deployment, api_key, endpoint, api_version, analysis_text)
    else:
...
def main():
    ...
    parser.add_argument(
        "--provider",
        "-p",
        choices=["openai", "azure", "bedrock"],
        help="Force provider (auto-detected from model name / env vars if omitted)",
    )
    parser.add_argument(
        "--aws-region",
    ...
    parser.add_argument(
        "--azure-endpoint",
        help="Azure OpenAI endpoint, e.g. https://my-resource.openai.azure.com "
             "(default: from AZURE_OPENAI_ENDPOINT env var)",
    )
    parser.add_argument(
        "--azure-api-version",
        help="Azure OpenAI API version, e.g. 2024-10-21 "
             "(default: from AZURE_OPENAI_API_VERSION env var)",
    )
    parser.add_argument(
        "--json",
        action="store_true",
    ...
    parser.add_argument(
        "--api-key",
        help="API key (OPENAI_API_KEY for OpenAI, AZURE_OPENAI_API_KEY for Azure). "
             "Can also be set via the corresponding env var.",
    )
    parser.add_argument(
        "--list-tasks",
        action="store_true",
