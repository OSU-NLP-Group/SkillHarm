...
def _patch_exec_as_agent(defense_text: str) -> None:
    ...
    def _build_gemini_inject(model_for_dump: str) -> str:
        ...
        The dump call costs ~$0.005-0.01 per sample and adds ~5s wallclock;
        across 206 samples that's ~$2 / ~3 min total at 10 parallel.
        """
        # Single bash line (semicolon-chained) up until the heredoc — the
        # heredoc must own its own newlines per shell grammar.
        return (
